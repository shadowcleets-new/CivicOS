...
**Learning:** Empty `onPressed` callbacks on stubbed buttons create a deceptive UI state where users might think the app is broken or silently failing.
**Action:** Always provide transparent messaging (e.g., using `SnackBar` with "Feature coming soon") for unimplemented features to set proper user expectations and avoid UX regressions.
