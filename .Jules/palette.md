...
## 2024-05-24 - Screen Reader Announcements for Dynamic Text
**Learning:** Text widgets that update asynchronously or simulate dynamic processes (like "AI Analyzing: Pothole Detected...") are not automatically announced by screen readers. Users relying on assistive technologies miss these real-time status updates unless explicitly instructed.
**Action:** Wrap dynamically updating text widgets in `Semantics(liveRegion: true, child: ...)` to ensure screen readers announce asynchronous state changes and provide equal access to dynamic information.
