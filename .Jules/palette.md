...
## 2026-08-07 - Transparent Messaging for Unimplemented Features
**Learning:** Leaving `onPressed` or `onClick` callbacks completely empty for stubbed features leads to silent failures, which is confusing and frustrating for users. They may think the app is broken or unresponsive.
**Action:** When a feature is not yet implemented, always provide explicit, transparent messaging (e.g., using a SnackBar, Toast, or Dialog) such as 'Feature coming soon' instead of failing silently.
