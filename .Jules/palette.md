...
**Learning:** When developing UI, leaving stubbed or unimplemented features (like an empty onPressed callback) causes silent failures and user confusion.
**Action:** Add explicit, transparent messaging (such as a 'Feature coming soon' SnackBar or Toast) to inform users when a feature is not yet available, rather than failing silently.
