...
## 2024-07-06 - Transparent Messaging for Stubbed Buttons
**Learning:** Silent or empty callbacks on interactive elements create a deceptive user experience, leading users to believe the app is broken. When buttons are implemented for features that are not yet active, they should clearly communicate their status.
**Action:** Always provide explicit, transparent feedback (e.g., 'Feature coming soon' SnackBar) for stubbed buttons or unimplemented features instead of fake success messages or no feedback.
