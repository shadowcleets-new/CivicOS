...
def read_grievances(limit: int = 100, cursor: uuid.UUID = None, db: Session = Depends(get_db)):
    ...
    if cursor:
        # Bolt Optimization: Use .scalar() to fetch only the created_at timestamp instead of hydrating the full Grievance entity to save memory and DB overhead during cursor resolution.
