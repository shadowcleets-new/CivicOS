...
def read_grievances(limit: int = 100, cursor: uuid.UUID = None, db: Session = Depends(get_db)):
    ...
    if cursor:
        # ⚡ Bolt: Optimize cursor fetch by selecting only the required column, reducing memory and I/O overhead.
        cursor_data = db.query(Grievance.created_at).filter(Grievance.id == cursor).first()
        if cursor_data:
            cursor_created_at = cursor_data[0]
            query = query.filter(
                or_(
