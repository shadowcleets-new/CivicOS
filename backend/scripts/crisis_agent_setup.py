...
def download_docs():
    ...
    for name, url in DOCS.items():
        ...
        try:
            ...
            response = requests.get(url, headers=headers, stream=True, verify=True, timeout=30) # verify=True to prevent SSL downgrade attacks
            response.raise_for_status()
            with open(path, 'wb') as f:
                f.write(response.content)
            logger.info(f"Downloaded {name}.")
        except requests.exceptions.SSLError as e:
            logger.error(f"SSL Certificate verification failed for {name}. Skipping to prevent MitM attack: {e}")
