...
def download_docs():
    ...
    for name, url in DOCS.items():
        ...
        try:
            ...
            response = requests.get(url, headers=headers, stream=True, verify=True, timeout=30) # Enforce SSL verification
            response.raise_for_status()
            with open(path, 'wb') as f:
                f.write(response.content)
            logger.info(f"Downloaded {name}.")
        except requests.exceptions.SSLError as e:
            logger.warning(f"SSL Certificate verification failed for {name}: {e}. Skipping resource.")
