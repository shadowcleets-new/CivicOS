...
def test_read_grievances_with_data(client, db):
    ...
    data = response.json()
    assert len(data) == 2
    # The endpoint sorts by created_at desc, id desc. Both are inserted practically at the same time.
...
def test_read_grievances_pagination(client, db):
    ...
    # Test offset is NOT supported by the endpoint (it takes cursor and limit).
    # We will test limit alone here.

    # Test limit only since cursor is UUID
    response = client.get("/api/v1/grievances/?limit=5")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 5

    # The endpoint does not support skip. Testing limit beyond total (there are 15 entries)
    # Using cursor to paginate. Since we can't easily get the middle cursor in this simple test,
    # we'll just test that limit=20 returns all 15.
