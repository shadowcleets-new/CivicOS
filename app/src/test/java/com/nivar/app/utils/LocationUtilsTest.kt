...
class LocationUtilsTest {
    ...
    fun setUp() {
        ...
        `when`(mockContext.applicationContext).thenReturn(application)

        // For simple tests not using the mocked static context, initialize directly
        locationUtils = LocationUtils(application)
    }
    ...
    fun getApproximateLocation_permissionDenied_returnsNull() = runTest {
        val localMockContext = Mockito.mock(Context::class.java)

        // Ensure mockContext returns application context for Robolectric
        `when`(localMockContext.applicationContext).thenReturn(RuntimeEnvironment.getApplication())
        ...
        Mockito.mockStatic(LocationServices::class.java).use { mockedLocationServices ->
            mockedLocationServices.`when`<FusedLocationProviderClient> {
                LocationServices.getFusedLocationProviderClient(localMockContext)
            }.thenReturn(mockFusedLocationClient)

            val localLocationUtils = LocationUtils(localMockContext)
            Mockito.mockStatic(ContextCompat::class.java).use { mockedContextCompat ->
                mockedContextCompat.`when`<Int> {
                    ContextCompat.checkSelfPermission(localMockContext, Manifest.permission.ACCESS_COARSE_LOCATION)
                }.thenReturn(PackageManager.PERMISSION_DENIED)

                val result = localLocationUtils.getApproximateLocation()
                assertNull(result)
            }
        }
    }
