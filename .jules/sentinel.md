...
## 2024-05-25 - Enforce SSL Certificate Validation
**Vulnerability:** Disabling SSL certificate verification (`verify=False`) in `requests.get`.
**Learning:** Disabling SSL verification allows for Man-in-the-Middle (MitM) attacks. Attempting to fallback to `verify=False` on `SSLError` is a downgrade attack.
**Prevention:** Always enforce SSL validation (default `verify=True`) and handle `SSLError` gracefully by logging and skipping the resource.
