...
## 2024-05-24 - Enforce SSL Certificate Validation
**Vulnerability:** Disabled SSL certificate validation (verify=False) when making HTTP requests, allowing potential man-in-the-middle attacks.
**Learning:** verify=False was used as a shortcut to bypass bad certificates on government websites.
**Prevention:** Always enforce SSL validation (verify=True) and handle SSLError gracefully by skipping the resource or using a proper CA bundle rather than disabling verification completely.
