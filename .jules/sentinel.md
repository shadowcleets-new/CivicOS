...
**Vulnerability:** Use of `verify=False` in `requests.get()` when downloading PDF files.
**Learning:** Disabling SSL verification (`verify=False`) leaves the application vulnerable to Man-in-the-Middle (MitM) attacks. Attempting to bypass it or falling back securely is an anti-pattern.
**Prevention:** Never disable SSL verification. Wrap network calls in a try-except block catching `requests.exceptions.SSLError` to handle bad certificates gracefully without crashing or bypassing security.
