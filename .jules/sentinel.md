...
## 2024-07-20 - Enforce SSL Certificate Validation
**Vulnerability:** The script bypassed SSL certificate validation by setting `verify=False` in requests, exposing it to man-in-the-middle attacks.
**Learning:** Even when interacting with sites known for bad certificates, never bypass SSL validation. Security theater such as bypassable try-catch blocks is harmful; it's better to log the failure and skip the resource.
**Prevention:** Always enforce SSL validation. Handle `SSLError` gracefully by skipping the resource if necessary rather than degrading security.
