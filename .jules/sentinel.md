...
**Vulnerability:** The application was bypassing SSL certificate validation by setting `verify=False` during HTTP requests, leaving it vulnerable to Man-in-the-Middle (MitM) attacks.
**Learning:** Never bypass SSL verification, even for government sites that may have bad certificates, as this fundamentally compromises the security of data transmission and facilitates downgrade attacks.
**Prevention:** Remove `verify=False` and explicitly catch `requests.exceptions.SSLError` to handle certificate verification failures securely, skipping the operation and logging an error rather than bypassing security protocols.
